...
const ComplaintCard = ({ complaint, onClick }) => (
  <div
    onClick={() => onClick && onClick(complaint.id)}
    ...
    <div className="px-5 pt-4 pb-3 flex-1 flex flex-col">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div
              ...
              style={{ fontFamily: "Montserrat, sans-serif" }}
            >
              {complaint.recipientRole}&nbsp;•&nbsp;{getTimeAgo(complaint.createdAt)}
            </div>
          </div>
        </div>

        {/* Right: bell + complaintId tag + status tag */}
      ...
      <div className="flex items-start gap-3 mt-2 flex-1">
        ...
        {complaint.hasAttachment ? (
          <AttachmentBox type={complaint.attachmentType} />
        ) : (
          <CalendarOutlined
    ...
    <div className="bg-gray-50 border-t border-gray-100">
      ...
      <div className="grid grid-cols-5 divide-x divide-gray-200 py-3">
        ...
        <div className="flex flex-col items-center gap-0.5 px-1">
          ...
          <div className="flex items-center gap-1 justify-center">
            ...
            >
              {complaint.complainOf?.name?.split(" ")[0] || "—"}
            </span>
          </div>
        </div>
        ...
        {/* Created */}
        <div className="flex flex-col items-center gap-0.5 px-1">
          <span style={cellLabel}>Created</span>
          <span style={cellValue}>{getTimeAgo(complaint.createdAt)}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 divide-x divide-gray-200 border-t border-gray-100 py-2">
        <div className="flex flex-col items-center gap-0.5 px-2">
          ...
          <span style={cellValue}>{formatDate(complaint.createdAt)}</span>
        </div>
        <div className="flex flex-col items-center gap-0.5 px-2">
          <span style={cellLabel}>Update Date</span>
          <span style={cellValue}>{formatDate(complaint.updatedAt)}</span>
        </div>
      </div>
    </div>
  </div>
