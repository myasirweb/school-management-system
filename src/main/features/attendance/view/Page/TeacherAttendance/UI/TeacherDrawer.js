...
const TeacherDrawer = ({ visible, teacher, onClose }) => {
  if (!teacher) return null;

  const pctColor = getStrokeColor(teacher.attendancePercentage);

  return (
    <Drawer
      title={
        <div className="flex items-center gap-3">
          <Avatar src={teacher.avatar} size={36} />
          <div>
            <div
    ...
    >
      ...
      <div className="flex items-center gap-5 mb-6">
        <Progress
          type="circle"
          percent={teacher.attendancePercentage}
        ...
        <div className="grid grid-cols-3 gap-2 flex-1">
          {[
            { label: "Present", value: teacher.presentDays, color: "rgb(100,196,178)" },
            { label: "Absent",  value: teacher.absentDays,  color: "rgb(232,19,123)" },
            { label: "Late",    value: teacher.lateDays,     color: "rgb(247,212,71)" },
          ...
          return (
            <div key={m.month} className="mb-2.5">
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="text-gray-500">{m.present} / {total} ({pct}%)</span>
              </div>
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${pct}%`,
                    backgroundColor:
                      pct >= 90 ? "rgb(100,196,178)" :
                      pct >= 75 ? "rgb(247,212,71)" : "rgb(232,19,123)",
                  }}
                />
              </div>
            </div>
          ...
          {teacher.dailyRecords.map((rec) => {
            ...
            return (
              <div
                ...
                className="w-7 h-7 rounded-md flex items-center justify-center text-xs font-semibold text-white cursor-default hover:opacity-80 transition-all duration-150"
