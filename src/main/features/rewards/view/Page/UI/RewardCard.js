...
const RewardCard = ({ reward, onClick }) => (
  <div
    onClick={() => onClick && onClick(reward.id)}
    ...
    <div className="px-5 pt-4 pb-3 flex-1 flex flex-col">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div
              ...
              style={{ fontFamily: "Montserrat, sans-serif" }}
            >
              {reward.recipientRole}&nbsp;•&nbsp;{getTimeAgo(reward.createdAt)}
            </div>
          </div>
        </div>

        {/* Right: bell + rewardId tag + status tag */}
      ...
      <div className="flex items-start gap-3 mt-2 flex-1">
        ...
        {reward.hasAttachment ? (
          <AttachmentBox type={reward.attachmentType} />
        ) : (
          <CalendarOutlined
    ...
    <div className="bg-gray-50 border-t border-gray-100">
      ...
      <div className="grid grid-cols-2 divide-x divide-gray-200 border-t border-gray-100 py-2">
        <div className="flex flex-col items-center gap-0.5 px-2">
          ...
          <span style={cellValue}>{formatDate(reward.createdAt)}</span>
        </div>
        <div className="flex flex-col items-center gap-0.5 px-2">
          <span style={cellLabel}>Update Date</span>
          <span style={cellValue}>{formatDate(reward.updatedAt)}</span>
        </div>
      </div>
    </div>
  </div>
