...
/* ── WarningCard ──────────────────────────────────────────── */
const WarningCard = ({ warning, onClick }) => (
  <div
    onClick={() => onClick && onClick(warning.id)}
    ...
    <div className="px-5 pt-4 pb-3 flex-1 flex flex-col">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            >
              {warning.recipientName}
            </div>
            <div
              className="text-xs text-gray-400 truncate"
              style={{ fontFamily: "Montserrat, sans-serif" }}
            >
              {warning.recipientRole}&nbsp;•&nbsp;{getTimeAgo(warning.createdAt)}
            </div>
          </div>
        </div>

        {/* Right: bell + warningId tag + status tag */}
      ...
      <div className="flex items-start gap-3 mt-2 flex-1">
        ...
        {warning.hasAttachment ? (
          <AttachmentBox type={warning.attachmentType} />
        ) : (
          <CalendarOutlined
    ...
    <div className="bg-gray-50 border-t border-gray-100">
      ...
      <div className="grid grid-cols-2 divide-x divide-gray-200 border-t border-gray-100 py-2">
        <div className="flex flex-col items-center gap-0.5 px-2">
          ...
          <span style={cellValue}>{formatDate(warning.createdAt)}</span>
        </div>
        <div className="flex flex-col items-center gap-0.5 px-2">
          <span style={cellLabel}>Update Date</span>
          <span style={cellValue}>{formatDate(warning.updatedAt)}</span>
        </div>
      </div>
    </div>
  </div>
